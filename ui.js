...
function renderActionButtons() {
    ...
    container.innerHTML = `
        ...
        <button onclick="endGameVote()" style="border-color:#ff4136;color:#ff4136">🏁 Завершити гру</button>
    `;
}
...
function showPropertyCard(pos) {
    ...
    if (c.type === 'property') {
        if (s.houses === 5) {
            housesHTML = `<div style="text-align:center;margin:8px 0;font-size:18px">🏨 <b style="color:#ff4136">Готель</b></div>`;
        } else if (s.houses > 0) {
    ...
    content.innerHTML = `
        <div class="property-card-color" style="background:${c.color || '#003d80'}">
            ${c.icon || ''} ${c.city || ''}
        </div>
...
function showAuctionUI() {
    ...
    if (!a || a.active.length === 0) {
        if (a) log('Аукціон завершився без покупця', 'warn');
        auctionState = null;
        closeModal();
        saveGame();
        showEndTurnBtn();
    ...
    showModal({
        ...
        buttons: [
            { text: 'Зробити ставку', class: 'btn-success', action: () => {
                ...
                closeModal();
                // Якщо ставник єдиний в active — він виграє
                if (a.active.length === 1) { _awardAuctionLocal(a); return; }
            ...
            { text: 'Пас', class: 'btn-secondary', action: () => {
                ...
                closeModal();
                // Якщо залишився 1 і це той хто зробив останню ставку — виграє
                if (a.active.length === 1 && a.active[0] === a.currentBidder) {
                    _awardAuctionLocal(a); return;
                }
...
function announceWinner(player) {
    ...
    showModal({
        ...
        body: `<div style="text-align:center;padding:20px">
            <div style="font-size:60px">${player.icon}</div>
            <h3 style="color:${player.color};font-size:24px;margin:10px 0">${player.name}</h3>
            <p style="font-size:18px">став єдиним монополістом!</p>
            <p style="font-size:22px;color:#2a9d3f;margin-top:14px">Капітал: ₴${calcNetWorth(player)}</p>
        </div>`,
        buttons: [
            { text: 'Нова гра', class: 'btn-primary', action: () => location.reload() }
        ]
    });
}
...
// ============================================
function showTaxModal(cell, reason) {
...
function renderRentModal() {
    ...
    const heroRent = `
        <div style="text-align:center;padding:20px 16px;margin-bottom:12px;
                    ...
                    border:2px solid ${canPay ? '#ffcccc' : '#ff4136'};
                    border-radius:14px">
