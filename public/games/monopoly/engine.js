...
function rollDice() {
    ...
    setTimeout(() => {
        ...
        log(`🎲 ${player.name} кинув ${d1} + ${d2} = ${total}${isDouble ? ' (дубль!)' : ''}`);
...
function movePlayerSteps(player, steps) {
    ...
    if (oldPos + steps >= 40 && oldPos !== 0) {
        // Пройшли через СТАРТ
        addMoney(player, 200);
        log(`💰 ${player.name} пройшов через СТАРТ. +₴200`, 'success');
    }
...
function animateTokenTo(player, newPos) {
    ...
    const intervalId = setInterval(() => {
        ...
        if (stepCount < steps) {
            ...
            // дочекатись завершення CSS-переходу останнього кроку
            setTimeout(() => {
                renderPlayers();
...
function handleLanding(player) {
    ...
    log(`📍 ${player.name} зупинився на: ${cell.name}`);
    ...
    if (cell.type === 'property' || cell.type === 'railway' || cell.type === 'utility') {
        ...
        showEndTurnBtn();
    } else if (cell.pos === 10 || cell.pos === 20) {
        showEndTurnBtn();
    } else if (cell.pos === 30) {
        const jailReasons = [
...
function handleJailRoll(player, d1, d2) {
    ...
    if (d1 === d2) {
        log(`🔓 ${player.name} викинув дубль і виходить з В'язниці!`, 'success');
        player.inJail = false;
...
function drawCard(player, type) {
    ...
    showModal({
        ...
        buttons: [
            { text: 'Ок', class: 'btn-primary', action: () => {
                closeModal();
                card.action(player);
                renderPlayers();
                placeTokens();
...
function startAuction(cell) {
    const decliningPlayer = players[currentPlayerIndex];
    const eligible = players.filter(p => !p.bankrupt && p.id !== decliningPlayer.id);
    ...
    auctionState = {
        ...
        currentBidder: null,
        active: eligible.map(p => p.id),
        turnIdx: 0
    };
    log(`🔨 ${decliningPlayer.name} відмовився — аукціон серед інших гравців`, 'warn');
    showAuctionUI();
}
...
function declareBankrupt(player, creditor, amount) {
    log(`💀 ${player.name} оголосив банкрутство!`, 'error');
    playSound('error');
    if (creditor) {
        addMoney(creditor, player.money);
        // передати власність кредитору
